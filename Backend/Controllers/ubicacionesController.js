...
const getAll = async function (req, res) {
    try {
        let p = await ubicacionModel.findAll(); //Busca todos los elementos
...
const getById = async function (req, res) {
    try {
        let p = await ubicacionModel.findByPk(req.params.id); //Busca por llave primaria (id)
...
const saveElement = async function (req, res) {
    ...
    try {
        let p = await ubicacionModel.create(req.body); //Crea un elemento Activo nuevo
...
const replaceById = async function (req, res) {
    try {
        ...
        await ubicacionModel.update(req.body, { where: { id: req.params.id } }); //Actualiza completamente un elemento Activo por id
        let p = await ubicacionModel.findByPk(req.params.id); //Busca por llave primaria (id)
...
const deleteId = async function (req, res) {
    try {
        ...
        if (p.length === 0) {
            ...
            await ubicacionModel.destroy({ where: { id: req.params.id } }); //Elimina un Activo por su id
            await res.json(p);
        }
    } catch (err) {
        await res
