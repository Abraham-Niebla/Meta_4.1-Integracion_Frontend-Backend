...
const editedItem = ref({ ...defaultUbicacion }); //Copia del modelo de item
...
const getData = async () => {
  ...
  await fetch(urlUbicaciones, { method: "GET" })
    ...
    .then((data) => {
      ...
      ubicaciones.value = data.map((ubicacion) => ({
        id: ubicacion.id, // No se muestra, solo para relacionar objetos
        ubicacion: ubicacion.ubicacion,
      }));

      console.log("Ubicaciones otenidos!");
      console.log(ubicaciones.value);
    });
...
const editItem = async (item) => {
  editedIndex.value = ubicaciones.value.indexOf(item);
...
const deleteItemConfirm = async () => {
  try {
    ...
    let url = `${urlUbicaciones}/id/${editedItem.value.id}`;
    ...
    await ubicaciones.value.splice(editedIndex.value, 1);
...
const close = () => {
  dialog.value = false;
  editedItem.value = { ...defaultUbicacion }; //Copia del modelo de item
  editedIndex.value = -1;
};
...
const closeDeleteItem = () => {
  ...
  editedItem.value = { ...defaultUbicacion };
  editedIndex.value = -1;
};
...
const saveElement = async () => {
  ...
  if (typeof(editedItem.value.id) === "number") {
    try {
      //Guardar cambios en la ubicacion
      let url = `${urlUbicaciones}/id/${editedItem.value.id}`;
      ...
      Object.assign(ubicaciones.value[editedIndex.value], editedItem.value);
      close();
    }
    catch (error) {
  ...
  // Guarda un elemento nuevo
    ...
    try {
      //Guardar Nuevo elemento ubicacion
      let response = await fetch(urlUbicaciones, {
        method: "POST",
        body: JSON.stringify({
          id: editedItem.value.id,
          ubicacion: editedItem.value.ubicacion,
      ...
      ubicaciones.value.push({ ...editedItem.value });
      close();
    }
    catch (error) {
